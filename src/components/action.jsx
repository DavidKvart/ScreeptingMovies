...
///this is a genre components its the same for all of them so i putted the notes only in here
...
const DisplayAction = (props) => {
    ...
    let arraylenght=props.ganerData.length;//amount of movies in data base 
    let amountOfpages=Math.ceil(arraylenght/movieAmount);//amount of pages of the specific genre
    let tempArray=props.ganerData.slice( movieAmount*(pageNumber-1),  movieAmount*pageNumber)// the array containing only the movies that needs to be shown on the courent page 
    ...
    return (
        ...
        <tbody className="tableBody">
            {tempArray.map((movie)=>{
                let icon='error';
                if(movie.like)
                    icon=<i onClick={(id,genre)=> props.changeLike(movie.id,'action')} className="bi bi-heart-fill"></i>;
                else
                    icon=<i onClick={(id,genre)=> props.changeLike(movie.id,'action')} className="bi bi-heart"></i>;
                ...
                return(
                    ...
                    <td>{movie.rating}</td>
                    <td>{icon}</td>
                    <td><button className="btn btn-outline-dark" onClick={(id,genre)=> props.del(movie.id,'action')} >remove</button></td>
                    </tr>
